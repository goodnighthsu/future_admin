            ...
            render: (item: SysRoleModel) => {
                return (
                    <div className={styles.cell_action}>
                        <Tooltip title='编辑'>
                            <Button className={styles.cell_edit} type='link' icon={<EditOutlined/>}  disabled={item.id===1}
                                onClick={() => {setSelectedRole(item); setIsOpenAdd(true)}}/>
                        </Tooltip>
                        <Tooltip title='设置'>
                            <Button className={styles.cell_edit} type='link' icon={<SettingOutlined/>} disabled={item.id===1}
    ...
    return (
        <PageContainer>
            ...
            <div className={styles.toolbar}>
                <Button type='primary' onClick={() => {setSelectedRole(undefined); setIsOpenAdd(true)}}>创建</Button>
            ...
            {/* modal */}
            {/* sys role add */}
            <Modal title={selectedRole ?  '编辑' : '创建'} centered destroyOnClose footer={null} open={isOpenAdd}  width='400px'
