...
const TrainingRegression: React.FC = (props) => {
  ...
  const createModel = async () => {
    ...
    const inputLayer: Dense = tf.layers.dense({ inputShape: [1], units: 1, useBias: true });
    _model.add(inputLayer);
    for (let index = 0; index < 6; index++) {
      // sigmoid relu
      _model.add(tf.layers.dense({ units: 50, activation: 'relu', useBias: true }));
    }
